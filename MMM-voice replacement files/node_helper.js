    ...
    //        exec('/opt/vc/bin/tvservice -o', null);
     //       this.hdmi = false;
        ...
        // You have to add these for your words (example LUCY)
            ...
            this.sendSocketNotification('SHOW_GAS');
        } else if (/(HIDE)/g.test(data) && /(GAS)/g.test(data)) {
            this.sendSocketNotification('HIDE_GAS');
        }
        ...
        else if (/(SHOW)/g.test(data) && /(SCORES)/g.test(data)) {
            this.sendSocketNotification('SHOW_SCORES');
        } else if (/(HIDE)/g.test(data) && /(SCORES)/g.test(data)) {
            this.sendSocketNotification('HIDE_SCORES');
        }
           ...
           this.sendSocketNotification('SHOW_WIND');
       } else if (/(HIDE)/g.test(data) && /(WIND)/g.test(data)) {
           this.sendSocketNotification('HIDE_WIND');
       }
